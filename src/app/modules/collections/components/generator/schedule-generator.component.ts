...
import { CollectionsSimplifiedService, ContractWithFinancing } from '../../services/collections-simplified.service';
import { GenerateScheduleRequest } from '../../models/payment-schedule';
                ...
                >
                  <div class="flex justify-between items-start">
                    ...
                    <div class="text-right">
                      <p class="text-sm font-medium text-gray-900">{{ formatCurrency(contract.financing_amount) }}</p>
                      <p class="text-xs text-gray-500">{{ contract.term_months }} meses</p>
                    </div>
                  </div>
                </div>
              }
              ...
              <div class="bg-blue-50 p-4 rounded-lg">
                ...
                <div class="grid grid-cols-2 gap-4 text-sm">
                  ...
                  <div>
                    ...
                    <p class="font-medium text-blue-900">{{ formatCurrency(selectedContract()!.financing_amount) }}</p>
                  </div>
                  <div>
...
export class ScheduleGeneratorComponent implements OnInit {
  ...
  // Signals
  contracts = signal<ContractWithFinancing[]>([]);
  selectedContract = signal<ContractWithFinancing | null>(null);
  ...
  selectContract(contract: ContractWithFinancing) {
  ...
  generateSchedule() {
    ...
    this.collectionsService.generatePaymentSchedule(request)
      .pipe(
        takeUntil(this.destroy$),
        catchError(error => {
      ...
      .subscribe({
        next: (response) => {
          if (response) {
            this.generatedScheduleId = response.schedule_id;
